...
class BlockedEmail(db.Model):
    """Addresses that must never receive mail and must never hold an account.

    Two separate guarantees, both enforced from this one table:

      * no mail — checked inside email_utils.send_email_with_details(), which
        is the single choke point every sender routes through (digest,
        outreach, transactional, manual admin sends). Blocking at the
        transport rather than in each caller is deliberate: a new sender
        added later inherits the block without anyone remembering to wire it
        up, which is the only way a rule like this stays true.

      * no account — checked in oauth._get_or_create_oauth_user() (the shared
        path for all three providers) and in the password login/register
        routes. Deleting a users row is NOT sufficient on its own: OAuth
        sign-in recreates an account from the provider profile, so a deleted
        user is one Google click away from being back, and back on the digest
        recipient list with it.

    The address is stored here, as data, and never written into source: this
    repository is public, so a denylist hardcoded in code would publish the
    very addresses it exists to protect. `BLOCKED_EMAILS` (comma-separated
    env var) is honoured alongside this table for the same reason — it lets
    an address be blocked without a deploy and without a commit.

    `reason` is free text for whoever reads this in six months. `created_at`
    is when the block was applied, not when the account was removed.
    """
