...
@api_bp.post("/generate-study-plan")
def api_generate_study_plan():
    course_name = request.form.get("course_name") or request.json.get("course_name") if request.is_json else request.form.get("course_name")
    duration_days = request.form.get("duration_days") or (request.json.get("duration_days") if request.is_json else None)
    hours_per_day = request.form.get("hours_per_day") or (request.json.get("hours_per_day") if request.is_json else None)
    topics = request.form.get("topics") or (request.json.get("topics") if request.is_json else "")
    priority_topics = request.form.get("priority_topics") or (request.json.get("priority_topics") if request.is_json else "")
    confidence_level = request.form.get("confidence_level") or (request.json.get("confidence_level") if request.is_json else "Intermediate")

    # Set default values if None or empty
    if not course_name:
        course_name = "Self-Guided Study Course"
    ...
    try:
        hours_per_day = int(hours_per_day) if hours_per_day else 2
    except ValueError:
        hours_per_day = 2
    ...
    if file:
        ...
        if ext in [".png", ".jpeg", ".jpg"]:
            image_bytes = file.read()
            mimetype = file.content_type or f"image/{ext[1:]}"
        else:
            from app.services.syllabus_parser import extract_text_from_file
    ...
    from app.services.study_plan_generator import process_and_save_study_plan
    result = process_and_save_study_plan(
        ...
        syllabus_image_bytes=image_bytes,
        syllabus_image_mimetype=mimetype
    )
...
@api_bp.get("/study-plans/<share_id>")
def api_get_study_plan(share_id: str):
    from app.models import StudyPlan
    db_plan = StudyPlan.query.filter_by(share_id=share_id).first()
    if not db_plan:
        return jsonify({"error": "Study plan not found"}), 404
    
    return jsonify({
        "share_id": db_plan.share_id,
